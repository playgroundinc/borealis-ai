...
if (!function_exists('pg_generate_hero_header')) {
    function pg_generate_hero_header($id, $headline)
    {
        ...
        ob_start();
?>
        <div class="flex flex-col grow md:min-h-350px">
            ...
            <div class="w-full grow md:w-6/12 relative pt-30 text-shade-white-400 flex flex-col justify-end">
                <h1 class="h1 md:h1-desktop pb-10 md:pb-19"><?php echo isset($headline) && strlen($headline) > 0 ? wp_kses($headline, 'post') : esc_html(the_title()); ?></h1>
            </div>
        </div>
<?php
        return ob_get_clean();
