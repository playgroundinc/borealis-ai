...
if ( ! function_exists( 'pg_render_gallery_container_block' ) ) {
    ...
    function pg_render_gallery_container_block( $attrs, $content, $block_obj ) {
        ...
        <div class="custom-component animated-element">
            <div class="md:flex container">
                <div class="md:w-4/12">
                    <?php if (!empty($attributes->title)): ?>                   
                        <h2 class="h2 pr-8"><?php echo esc_html($attributes->title) ?></h2>
                    <?php endif; ?>
                </div>
                <div class="md:w-8/12">
            ...
            <div>
                <div class="mx-auto relative pt-gallery">
                    <ul class="flex absolute left-0 top-0 bottom-0 max-w-full overflow-scroll pl-gallery-offset pr-25">
                        <?php foreach ( $block['innerBlocks'] as $index => $inner_block ) : ?>
                            ...
                            <?php elseif ($inner_block['blockName'] === $namespace . '/select-team-member'): ?>
                                ...
                                <li class="nth-child-2:ml-5  basis-team-member shrink-0 grow-0">
                                    <a href="<?php echo esc_url_raw($url)?>" class="block">
                                        <img class="block rounded-large" src="<?php echo esc_url_raw($image); ?>"/>
                                        <p class="h4 pt-12"><?php echo esc_html($post->post_title); ?></p>
