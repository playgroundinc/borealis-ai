    ...
    function pg_render_products_item($block, $link) {
        ...
        ?> 
            <li class="block border-b border-color-shade-grey-500">
                <a class="block py-15 hover:bg-shade-grey-100 bg-shade-white-400 transition-background-color duration-300 group" href="<?php echo esc_attr(get_permalink($post->ID))?>">
                    <div class="container flex items-center">
                        ...
                        <div class="basis-1/2 shrink-0 relative">  
                            <p class="paragraph"><?php echo $description && strlen($description) > 0 ? esc_html($description) : esc_html($post->post_title); ?></p>
                            <div class="opacity-0 invisible absolute rounded-large overflow-hidden right-6 -top-20 group-hover:opacity-100 group-hover:visible transition-all duration-400">
                                <img src="<?php echo esc_url_raw($image)?>" />
                            </div>
                        </div>
                        <div class="basis-1/6 shrink-0">
                            <p class="primary-button flex items-center"><?php echo esc_html('View Product') ?><span class="pl-8"><?php echo pg_render_icon('arrow-white'); ?></span></p>
                        </div>
                    </div>
                </a>
            </li>
